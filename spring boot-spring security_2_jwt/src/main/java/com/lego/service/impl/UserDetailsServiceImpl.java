...
public class UserDetailsServiceImpl implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String userName) throws UsernameNotFoundException {
        ...
        if (null != user) {
            ...
            user.setRoles(sysRoles.stream().map(SysRole::getRole).collect(Collectors.toList()));
            // 获取权限列表
            List<SysPermission> lstPermissions = permissionMapper.getPermissionsByUserId(user.getId());
            user.setPermissions(lstPermissions.stream().map(SysPermission::getPermission).collect(Collectors.toList()));
        }
        return user;
    }
}
